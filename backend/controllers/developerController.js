...
const getDeveloperProfile = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: "Failed to fetch developer profile" });
  }
};
...
const developerDashboardById = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT e.fullName, ei.*,ei.flag AS level, CASE WHEN b.id IS NOT NULL THEN 1 ELSE 0 END AS is_bookmarked
